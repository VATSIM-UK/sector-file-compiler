            ...
            for (int i = 0; i < data.lines.Count; i++)
            {
                ...
                if (sectorData.dataSegments.Count != 4)
                {
                    this.eventLogger.AddEvent(
                        new SyntaxError("Incorrect number of NDB segments", data.fileName, i)
                    );
                    continue;
                }
                ...
                {
                    this.eventLogger.AddEvent(
                        new SyntaxError("Invalid NDB identifier: " + sectorData.dataSegments[0], data.fileName, i)
                    );
                    return;
                }
                ...
                {
                    this.eventLogger.AddEvent(
                        new SyntaxError("Invalid NDB frequency: " + sectorData.dataSegments[1], data.fileName, i)
                    );
                    return;
                }
